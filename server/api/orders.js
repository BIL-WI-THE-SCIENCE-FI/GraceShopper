...
router.get('/update/:userId', [isCorrectUser], async (request, response, next) => {
  try {
    //* Get the user id
    const userId = request.params.userId

    //* Find user
    const order = await Order.getOpenOrder(userId)
...
router.get('/current/:userId', [isCorrectUser], async (request, response, next) => {
  try {
    //* Get the user id
    const userId = request.params.userId
...
router.post('/:userId', isCorrectUser, async (request, response, next) => {
  try {
    ...
    const orderInstance = await Order.getOpenOrder(userId)
...
router.get('/all/:userId', isCorrectUser, async (request, response, next) => {
  try {
    //* Get the user id
    const userId = request.params.userId
